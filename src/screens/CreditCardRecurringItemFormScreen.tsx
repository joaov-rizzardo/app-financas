...
import type { Category, RecurringCardItem } from '@/types/finance';
...
export interface CreditCardRecurringItemFormScreenProps {
  item: RecurringCardItem;
  categories: Category[];
  onSave: (id: string, data: Pick<RecurringCardItem, 'description' | 'amount' | 'categoryId'>) => Promise<void>;
  onBack: () => void;
}
...
export function CreditCardRecurringItemFormScreen({
  item,
  categories,
  onSave,
  onBack,
}: CreditCardRecurringItemFormScreenProps) {
  ...
  const typeColor = isInstallment ? colors.warning : colors.primary[400];
  const TypeIcon = isInstallment ? Layers : RefreshCw;
  const typeLabel = isInstallment ? 'Parcelamento' : 'Assinatura';
      ...
      >
        ...
        >
          ...
          {isInstallment && (
            <View style={{
              ...
              <Text size="xs" style={{ color: colors.warning, flex: 1 }}>
                Parcela {item.installmentCurrent ?? 1} de {item.installmentTotal} · Alterar o valor afeta apenas as próximas faturas.
              </Text>
            </View>
          )}
          ...
          <View style={{
            ...
            <Label className="mb-2">Valor por fatura</Label>
